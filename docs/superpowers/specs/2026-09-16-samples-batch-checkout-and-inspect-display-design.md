...
**2026-09-16 用户确认（方案 A，实施依据）**：`CHECKED_OUT` 与 `RELEASED` **不进「不适用」集合**（二者仍有复检语义），但列表 / 「复检到期」列 / 详情 / CSV 一律**灰化且不出红色徽章**——`CHECKED_OUT` 显示灰字「领用中·暂停复检」（复检日在领用期间到期也**不**报逾期），`RELEASED` 保留三态文字（正常/近7天到期/逾期N天）但灰化。故 §6.2 表中 `CHECKED_OUT` 行的「不适用」文案**以本行为准**（改为「领用中·暂停复检」）；「不适用」只留给状态层无复检语义的 4 个态。

### 6.2 呈现矩阵

| 状态 | 列表「复检状态」列 | 「复检到期」列（逾期/近 7 天视图） | 详情弹窗复检行 | CSV「复检状态」列 |
|---|---|---|---|---|
| `RELEASED` | 现状三态（**灰化**，不出红徽章） | 现状日期（**灰化**不出红） | 现状（**灰化**） | 现状 |
| `IN_CUSTODY` | 现状三态 | 现状 | 现状 | 现状 |
| `CHECKED_OUT` | 灰字「**领用中·暂停复检**」+ title「领用中，复检计划暂停」 | 「领用中·暂停复检」 | 「复检 领用中·暂停复检」 | 「领用中·暂停复检」 |
...
- 「不适用」用灰字（复用 `.muted`），**不得**使用徽章类，尤其**不得继承 `.b-overdue`**——该类带 1.5s 无限脉冲动画（`public/css/app.css:85`），已作废样品持续红色脉冲等于「仍在报警」；
  - **2026-09-16 线上实测校正（避免以讹传讹）**：存量 26 件 `RETIRED`、53 件 `CHECKED_OUT` 的 `next_inspect_at` **全部为未来日期**，故修复前它们显示的是**绿色「正常」**（C1 的失效点就是「已作废样品被判为正常」），**并非**已在红脉冲。「红脉冲」是 `CHECKED_OUT` 在领用期间复检日到点后**会**走入的路径（本轮以灰化从根上消除该可能），本条视觉约束作为**预防性**要求继续有效，不得据此声称修复前已存在红色报警。
...
| C2 | `RETURNING`/`NEW`/`PRODUCED` 同样显示「不适用」；`CHECKED_OUT` 显示灰字「领用中·暂停复检」；`RELEASED` 保留三态文字但灰化（2026-09-16 方案 A 定稿） |
