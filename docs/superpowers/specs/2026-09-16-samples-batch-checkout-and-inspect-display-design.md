...
| 决策记录 | ① 交互形态＝**扫码队列 + 公共设置前置**；② 失败语义＝**两阶段（预校验全或无 + 执行期逐件结果）**；③ 需求 2＝**只改展示层**（前端徽章 + 导出同口径），**不动数据**；④ 本轮只出文档，不改代码；⑤ 七项业务决策已于 2026-09-16 逐项确认（上限 **50 件**、**统一领用人 + 统一时长**、「本就已归还」记 **跳过**、`RETIRED` 显示 **「不适用」+ title 含作废原因**、**一期即引入 `batchId` 幂等校验**、**不新增跨部门门槛**、发布 **分两批**），见 §11 |
...
6. **幂等命中**单独标「已生效（无需重试）」，不计入失败：后端以 `batchId` 命中已执行批次时返回 409 `code=BATCH_DUPLICATE` + `applied:[编号]`，前端对队列逐件 `resolve` 复核——已生效件标「已生效」，未生效件以**新 `batchId`** 重新提交（避免重复执行已生效件）；
...
| 同批次重复提交（重试/双击/断网重发） | 409 + `code=BATCH_DUPLICATE` | 不提交（零副作用） | 否 | 逐件 `resolve` 复核：已生效件标「已生效」，其余件换新 `batchId` 重交 |
...
| POST | `/api/samples/batch-action` | 批量执行（body `{action:'CHECKOUT'\|'RETURN_OUT', codes:[], checkout_user, checkout_dept, durationHours, note, batchId}`，`batchId` **必填**） | 登录 + 逐件角色校验 |

- 上限 50，超出 → 400；
- `batchId` 必填（8~40 字符，`[A-Za-z0-9_-]`）：前端每次点「提交」生成 UUID，**同一次提交的重试复用同一值**（这是幂等键的全部意义）；缺失/非法 → 400；
...
- 批次号写入 `note` 尾部：`<单件原文> [batch:<batchId>]`（1 个空格分隔，**零 DDL**）。单件扫码路径不带该后缀，故单件 `note` 逐字节不变；`routes-checkout-users.js:19-26` 按正则提取「领用人 X（部门）」「应还 …」，后缀不影响匹配（R3 有断言）。是否加 `scan_logs.batch_no` 列 + 索引留待观察，不在本期。

### 5.5 幂等与并发

**幂等键（2026-09-16 用户确认，一期落地）**：

- 前端：队列级锁 + 每项状态位（防重复点击）；每次「提交」生成 `batchId`（UUID），重试沿用同一值；
- 后端：阶段 1 通过后、阶段 2 执行前做一次幂等探测 `SELECT 1 FROM scan_logs WHERE note LIKE CONCAT('%[batch:', ?, ']%') LIMIT 1`：
  - 命中 → 409 `{ code:'BATCH_DUPLICATE', batchId, applied:[...] }`，**整批零副作用**；
  - 未命中 → 进入逐件事务执行。
- 职责边界：`batchId` 只防「**同批**重复提交」；**跨批并发**仍由逐件 CAS 兜底（后到者 `VERSION_CONFLICT`）。两者互补，缺一不可，不可用其一替代其二。
- 探测代价：`note` 前置通配符导致该列全表扫描，但每批仅 1 次且 `LIMIT 1` 短路。`scan_logs` 当前行数与探测耗时 MUST 在 T3 实测并记入容量报告；若 >50ms，本迭代内改为「精确后缀匹配 + 进程内记忆化」，不引入 DDL。
...
| B5 | 批量与逐件路径对同一批数据**逐字段等价**：状态、`scan_logs` 条数一致；`note` 文本 = 单件原文 + 唯一后缀 ` [batch:<batchId>]`，其中「领用人 X（部门）」「应还 …」部分与单件逐字节一致 |
...
| B10 | 幂等：同一 `batchId` 二次提交 → 409 `BATCH_DUPLICATE`，`scan_logs` 零新增、样品状态零变化；缺失/非法 `batchId` → 400 |
| B11 | 跨批并发（两个不同 `batchId` 操作同一样品）→ 后到者 `VERSION_CONFLICT`，结果面板归「可重试」，无脏数据 |
...
| R3 | 领用人候选排序被日志格式变更破坏 | **高** | 批量 `note` = 单件原文 + ` [batch:<id>]` 后缀（原文任何一段都不改）；测试断言「领用人 X（部门）」「应还 …」正则仍可命中（`routes-checkout-users.js:19-26`） |
...
| R8 | `batchId` 幂等探测走 `note` 前置通配 → 全表扫描 | 中 | 每批仅 1 次 + `LIMIT 1`；T3 实测耗时与行数并记入容量报告；超 50ms 改精确后缀匹配 + 记忆化（仍无 DDL） |
...
8. 不新增 `scan_logs` 列、不新建表（`batchId` 记在 `note` 尾部；列化 + 索引留待观察）；
9. 不修改 `shared/`、`app.css`、`server.js`（无需双系统功能回归，仅做只读回归）；
10. 不改 `AGENTS.md`/`CLAUDE.md` 规则文件（容量台账更新需用户单独授权）。

---

## 十一、业务决策（2026-09-16 用户逐项确认，实现冻结依据）

| # | 事项 | 用户选择 | 落地位置 |
|---|---|---|---|
| Q1 | 一次批量上限 | **50 件**（前端即拒 + 后端 400 二次校验） | §4.2、§5.1、B8 |
| Q2 | 批内领用人/时长 | **统一领用人 + 统一时长**（公共设置一处填满，扫码阶段零输入） | §4.1、§4.2 |
| Q3 | 「本就已归还」/重复扫码件 | **记 `skipped` 并说明原因**，其余件正常执行 | §4.3、§4.4、B4 |
| Q4 | `RETIRED` 复检列文案 | **「不适用」+ `title` 含作废原因**（灰字、不得继承 `.b-overdue` 脉冲） | §6.1~§6.3、C1 |
| Q5 | `batchId` 幂等键 | **一期即引入**：必填、写入 `note` 尾部、执行前探测命中即 409 `BATCH_DUPLICATE` 零副作用 | §5.1、§5.4、§5.5、B5、B10、B11 |
| Q6 | 跨部门门槛 | **不新增**，沿用现有角色 + 柜位逐件校验 | §5.1、B7 |
| Q7 | 发布批次 | **分两批**：先需求 2（复检展示，T1→T2）独立上线，再需求 1（T0→T3→T4→T5） | 实现计划「批次划分」 |

> Q5 的落地细节（原文与后缀、探测时机、与 CAS 的职责边界）见 §5.4、§5.5；实现冻结以本节为准，后续如需变更 MUST 用户重新确认。
