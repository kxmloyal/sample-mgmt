...
| 流水号 | 3 | `001` ~ `999` | 按「机型」级分配（跨提供处 C/T/G、跨组别共享 001~999 空间），取最小未占用序号；**占用口径 2026-09-11 修订**：取消 NEW（建样未制作、无实物）→ 编号释放、可复用；取消 PRODUCED 及以后 → 序号仍视为占用、永不复用（软删行仍留在表内），无空档时推进 `sample_seqs` 序列表 |
...
2. 取号（**2026-09-11 修订**：取消 T13「软删一律不复用」，改为按样品状态分层——**取消 NEW 释放编号可复用；取消 PRODUCED 及以后仍占用、永不复用**）：
   - 确保序列表行存在（`INSERT ... ON DUPLICATE KEY UPDATE cur_seq = cur_seq`，no-op）；
   - 行锁读 `cur_seq`（`SELECT ... FOR UPDATE`，记录该机型已分配的最大值）；
   - 查询该机型**已占用**序号（`USED_SQL`：`SELECT sample_no FROM samples WHERE SUBSTRING(sample_no,3,6)=? AND (deleted_at IS NULL OR status <> ?)`，第二个参数为可释放状态 `NEW`——**已取消的 NEW 行不计入占用，其余软删行仍占用**，防止已制作样品的旧实物 QR 扫码指向新样品）；
   - 取**最小未占用序号**：已取消 NEW 释放出的号与物理缺失的号都是空档；无空档时 `UPDATE sample_seqs` 将 `cur_seq` 推进为新序号（`cur_seq` 只是水位、不是下限）。
...
- 兜底：`samples.sample_no` 有唯一索引（2026-09-11 起为「仅存活行唯一」的函数索引 `uk_sample_no_live`，迁移 `migrateSamplesSampleNoRelease`），`dao.js createSample` 对唯一键冲突（`ER_DUP_ENTRY`）以 SAVEPOINT 重试最多 3 次。
- **占用口径按状态分层**（2026-09-11 修订，取代 T13「一律不复用」）：删除样品为软删除（`deleted_at` 置位），样品行仍留在表内；**已取消且 status='NEW'（建样后未制作、无实物）→ 不占用，序号可被新样品复用**；**已取消且 status<>'NEW'（已制作，实物/标签可能在外）→ 仍占用、永不复用**。占用判定集中在 `sample-code.js` 的 `USED_SQL`（取号与编号预览共用同一常量，口径必然一致）；不依赖删除时回写 `cur_seq`，天然并发安全；外部直接 INSERT 带编号不更新序列表（已知限制）。
...
6. 序列表（`sample_seqs`）记录机型已分配的最大序号（**只是水位、不是下限**：占用情况以 `samples` 表为准，已取消的 NEW 行不再占用、其余软删行仍占用）；`samples.sample_no` 唯一索引口径为「**仅存活行唯一**」（`uk_sample_no_live`，迁移 `migrateSamplesSampleNoRelease`，使被取消的 NEW 编号可真正复用）；新建/预览/扫码逻辑改动 MUST 评估序列表一致性；部署顺序为「schema.sql 建表（重启自动）→ 初始化脚本回填存量 MAX → 新代码生效」。
