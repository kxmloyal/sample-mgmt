...
> **生效方式**：后端行为变更 → **需要一次服务重启**（由运维在宝塔面板「停止 → 启动」执行，**AI 未代执行**，§23.1）。**实际时序见 §6.1**：2026-09-16 运维本次重启发生在代码拉取**之前**，加载的仍是 v2.0.7 后端代码 → **须第二次重启才激活本版的自动释放**
...
| 时刻 | 事件 | 结果 |
|---|---|---|
| 11:16:06 | 运维在宝塔面板重启（**早于代码拉取**）| 进程 `1986303` 加载的是**磁盘上当时的 v2.0.7 后端代码**——即 v2.0.7 的独立清柜动作、`gone` 分桶**首次真正生效**；**本版自动释放尚未生效** |
...
```bash
ss -ltnp | grep :4000                      # 单实例，PID 应为新值（当前 1986303）
curl -s -o /dev/null -w '%{http_code}\n' http://127.0.0.1:4000/health   # 期望 200
grep -o '"version": "[0-9.]*"' subsystems/samples/manifest.json | head -1   # 期望 2.0.8
# 运行态判别（关键）：登录后取柜位图，响应不应再出现 gone
