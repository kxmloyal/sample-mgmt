...
样品编号第 2~7 位为机型短码（人工填入，如 `YD9015`），新建样品表单当前「机型」为文本输入、「规格/型号」为文本输入（placeholder 提示参考 BOM 表机型全称）。为统一机型口径、减少手输错误，新增「机型列表」管理能力：由研发部（RD）与管理员（ADMIN）维护机型主数据，新建样品时规格/型号改为下拉选择，列表页新增按机型筛选。
