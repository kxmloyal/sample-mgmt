...
async function viewUsers(){
  const v=$('#view');v.innerHTML='<div class="filters"><input id="u-user" placeholder="账号"/><input id="u-name" placeholder="姓名"/><select id="u-role"><option value="RD">研发 RD</option><option value="ME">生技 ME</option><option value="QA">品保 QA</option><option value="CUSTODY">保管 CUSTODY</option></select><input id="u-dept" placeholder="部门"/><input id="u-pass" placeholder="初始密码" value="123456"/><button class="btn sm" onclick="addUser()">新增账号</button></div><div id="u-list"></div>';
  loadUsers();
}
