...
function _renderAlertBlock(type, title, list, pager, pageFn, isOverdue) {
  ...
  var rows = pageList.map(function(s) {
    ...
    return '<fluent-data-grid-row class="dash-alert-row" onclick="viewDetail(\'' + s.id + '\')" style="cursor:pointer"><fluent-data-grid-cell>' + e(s.sample_no) + '</fluent-data-grid-cell><fluent-data-grid-cell>' + e(s.name || '—') + '</fluent-data-grid-cell><fluent-data-grid-cell>' + img + '</fluent-data-grid-cell><fluent-data-grid-cell>' + e(s.custody_dept || '—') + '</fluent-data-grid-cell><fluent-data-grid-cell>' + e(s.storage_location || '—') + '</fluent-data-grid-cell><fluent-data-grid-cell class="' + dateCls + '" style="' + dateStyle + '">' + fmt(s.next_inspect_at) + '</fluent-data-grid-cell><fluent-data-grid-cell><a class="link" onclick="event.stopPropagation();goScan(\'' + e(s.sample_no) + '\')">去处理</a></fluent-data-grid-cell></fluent-data-grid-row>';
  }).join('');
  var pagerHtml = _renderPager(pager, pageFn);
  return '<div class="' + cls + '" id="dash-' + type + '"><h3>' + title + '（' + list.length + '）</h3><div style="overflow-x:auto"><fluent-data-grid><fluent-data-grid-row row-type="header"><fluent-data-grid-cell cell-type="columnheader">编号</fluent-data-grid-cell><fluent-data-grid-cell cell-type="columnheader">名称</fluent-data-grid-cell><fluent-data-grid-cell cell-type="columnheader">图片</fluent-data-grid-cell><fluent-data-grid-cell cell-type="columnheader">保管部门</fluent-data-grid-cell><fluent-data-grid-cell cell-type="columnheader">储位</fluent-data-grid-cell><fluent-data-grid-cell cell-type="columnheader">' + (isOverdue ? '应复检日' : '到期日') + '</fluent-data-grid-cell><fluent-data-grid-cell cell-type="columnheader">操作</fluent-data-grid-cell></fluent-data-grid-row>' + rows + '</fluent-data-grid></div>' + pagerHtml + '</div>';
}
