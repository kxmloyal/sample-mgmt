...
function _renderSampleList(list,isOverdue,pager){
  ...
  var rows=list.map(function(s){
    ...
    return '<tr><td>'+e(s.sample_no)+'</td><td>'+e(s.name||'—')+'</td><td class="muted">'+e(s.model||'—')+(s.station?(' · '+e(s.station)):'')+'</td><td>'+imgCell+'</td><td class="muted">'+e(s.spec||'—')+'</td><td>'+typeCell+'</td><td>'+statusBadge(s)+'</td><td class="muted">'+fmt(s.produced_at)+'</td><td class="muted">'+fmt(s.released_at)+'</td><td class="muted">'+e(s.custody_dept||'—')+'/'+e(s.storage_location||'—')+'</td>'+overdueCell+'<td>'+actions+'</td></tr>';
  });
  var html='<div class="card" style="padding:0"><table>'+cols+rows.join('')+'</table></div>';
