...
**§25 强制检查点清单（2026-09-17 新增，AGENTS.md §25 / 参考卷 C）**——凡涉及写接口、DAO 写语句、前端视图、共享文件、构建与容量台账、规则或测试，MUST 逐条自检：

- [ ] **入参白名单**：本次新增/修改写入的**枚举/标签字段**逐个有服务端 `includes` 白名单（§25.2.1）
- [ ] **输出转义**：非本函数产出的字符串进 `innerHTML`/属性前均经 `e()`；无「标签函数原始值兜底」（§25.2.2）
- [ ] **错误文案**：`catch` 分支未透出 `err.message`，已改固定文案 + `logger.error`（§25.2.3）
- [ ] **CSV 单点**：未新增自写 `esc`，复用 `shared/csv.js`（§25.2.4）
- [ ] **幂等键载体**：机器可读标记未写入自由文本列；幂等键有 UNIQUE 约束（§25.3.1）
- [ ] **时钟同源**：时间窗口查询的列类型与比较函数匹配（§25.3.2）
- [ ] **校验器唯一**：业务不变量由单一校验器覆盖**全部**写入口；DAO 写语句含存活条件（§25.3.3、§25.3.6）
- [ ] **副作用收敛**：多出口动作的副作用由单一列表驱动函数施加，并有覆盖断言（§25.3.4）
- [ ] **状态机 `to`**：动作声明并断言了目标状态（§25.3.5）
- [ ] **manifest 单一来源**：未硬编码等价列表；漂移测试已覆盖新增项（§25.3.7）
- [ ] **跨子系统边界**：未借用其它子系统实现；DAO 名唯一性已用**运行期工厂展开**核验（§25.4.1）
- [ ] **共享代码使用方**：改动前已实测引用数并列出清单；未据文件名做合并（§25.4.2、§25.4.3）
- [ ] **容量口径**：用 LF 归一字符数 + 脚本重算，已标注口径（§25.5.1）
- [ ] **`register()` 豁免认知**：未把注册表行数当业务函数长度；顶层函数数（含隐式全局赋值）≤10（§25.5.2）
- [ ] **重建 bundle**：改动前端 JS 后已重建并同步 `index.html` 版本号（§25.5.3）
- [ ] **视图卸载协议**：涉及摄像头/全局监听/Observer/rAF/在途请求的视图已实现 `leave()`（§25.6.1）
- [ ] **字典单一来源**：未在视图内新建状态映射字面量（§25.6.2）
- [ ] **全局符号唯一**：无跨文件重复顶层声明；共享常量声明序在消费者之前（§25.6.3）
- [ ] **前端 lint**：改动未落在 ESLint 覆盖之外（§25.6.4）
- [ ] **seed 护栏在入口**：破坏性脚本的护栏行号早于首个破坏性语句（§25.6.5）

> **拦截**：上述任一项未通过 → 暂停提交，先补齐。完整规则正文、依据与检查点见 `docs/agents-enforcement-rules.md`。
...
- 表格/列表优先于大段文字
- 关键信息加粗
- 不用 emoji 除非用户要求（本条**仅约束 AI 回复文本**;产品 UI 文案的 emoji 依 AGENTS.md 第 10 节「UI 文案符号约定」保留,2026-09-14 用户确认）
- 不预测时间估算
...
## 21. 强制检查点与规则补充（Claude 实施指引）

> 完整规则正文（24 条 + 依据 + 可执行检查点）见 **AGENTS.md §25 / 参考卷 C：[docs/agents-enforcement-rules.md](./docs/agents-enforcement-rules.md)**。
> 来源：`docs/samples-review-2026-09-17.md` 专项评审（三路并行专家评审 + 主控逐行复核）。
> **动手前 MUST 先读该参考卷**：新增/修改任何写接口、DAO 写语句、前端视图、共享文件、构建与容量台账、规则或测试。

### 21.1 常驻红线（16 条，不查参考卷也必须遵守）

**安全与转义**：① 用户可写入的枚举/标签字段 MUST 服务端白名单化，禁止只 `.trim()`；② 非本函数产出字符串进 `innerHTML`/属性前 MUST 经 `e()`，标签映射函数禁止原始值兜底；③ `catch` 分支 MUST NOT 透出 `err.message`；④ CSV 生成 MUST 单点复用 `shared/csv.js`，禁止自写 `esc`。

**数据正确性**：⑤ 机器可读标记禁止写入用户自由文本列，幂等键 MUST 有 UNIQUE 约束；⑥ 超时谓词 MUST 时钟同源（`TIMESTAMP`×`NOW()` / ISO×`UTC_TIMESTAMP()`）；⑦ 一个业务不变量 = 一个校验器；DAO 写语句 MUST 自带存活条件；⑧ 多出口动作的副作用 MUST 单一列表驱动 + 覆盖断言；⑨ 状态机动作 MUST 声明并断言 `to`；⑩ `manifest.json` 是单一事实来源，MUST 有全量漂移测试。

**跨子系统边界**：⑪ 禁止跨子系统借用实现（含 `D.<fn>`）；DAO 名唯一性 MUST 用运行期工厂展开核验，禁止用 grep 代替；给借用方补同名导出不会生效 → MUST 三步独立提交；⑫ 改 `shared/` 前 MUST 先实测使用方数量；跨子系统同名文件不等于共享代码，禁止据文件名合并。

**容量与构建**：⑬ 容量判定 MUST 用 LF 归一字符数，台账 MUST 脚本重算并标注口径（禁止 `Get-Content`/`wc -c`/字节数）；⑭ `register()` 豁免 60 行限制；顶层函数 ≤10（含隐式全局赋值）；⑮ 改前端 JS 后 MUST 重建 bundle。

**前端与护栏**：⑯ 视图 MUST 声明卸载协议 `VIEWS[k].leave()`；字典中文名与颜色 MUST 单一来源；`frontend/**` MUST 被 ESLint 覆盖；破坏性脚本护栏 MUST 在函数入口。

### 21.2 Claude 禁止行为（§25 相关）

1. 新增写入口只做 `.trim()` 而无白名单 → 拒绝，要求补 `includes` 校验。
2. 标签映射函数用原始值兜底（`x==='A'?'甲':x`）→ 拒绝，要求兜底返回安全常量。
3. 直接给借用方补同名 DAO 导出即认为修好 → 暂停，说明 `db.js` 前缀改名陷阱，要求三步独立提交。
4. 用 `Get-Content`/`wc -c`/字节数判定容量越线 → 拒绝，要求改用权威口径脚本。
5. 改前端 JS 不重建 bundle → 拒绝，要求执行 §19 流程后再提交。

### 21.3 与 §14.6 的关系

本指引与参考卷 C 同属规则文件，**修改需用户明确同意**。`AGENTS.md` 受 65,536 字节注入预算约束，新增条文 MUST 评估预算（详见参考卷 C §25.7.3）。
