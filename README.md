...
- **角色相关数据范围（2026-09-07，服务端 scope）**：不同角色打开列表时默认只取与自身职责相关的数据（服务端按会话身份派生 `scope=role`，客户端不可伪造）——研发→**我创建的**（全状态）、品保→待发行/退回审核 **或** 在库复检7天内临期、保管/生技→在库/借出/归还中、管理员→全量；列表顶部橙色芯片提示「已按角色优先显示：××」，点 ✕ 即回全量（数据不隐藏，非行级权限）；搜索/翻页/其它筛选保持角色范围，状态下拉主动选择、快捷筛选或深链进入则解除；CSV 导出跟随范围口径
