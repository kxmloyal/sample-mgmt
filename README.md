...
- **角色默认优先显示（2026-09-07）**：不同角色打开列表时默认过滤到本角色最相关的状态——研发→待制作(NEW)、品保→制作完成+退回审核中(PRODUCED,RETURNING)、保管/生技→保管中(IN_CUSTODY)、管理员→全量；列表顶部橙色芯片提示「已按角色优先显示：××」，点 ✕ 即回全量（数据不隐藏，非行级权限）；搜索/翻页/其它筛选保持角色范围，状态下拉主动选择、快捷筛选或深链进入则覆盖角色默认
