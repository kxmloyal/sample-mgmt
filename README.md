...
> 详情弹窗交互采用框架级**设计系统**（`shared/frontend/detail-modal.js` + `public/css/app.css` 共享样式）：骨架屏加载、Tab 置顶、密度自适应宽度、未保存拦截、409 自动刷新、日志时间线。各子系统详情弹窗复用同一组件（规范见 `docs/archive/specs/2026-09-03-detail-modal-design-system.md`）。
...
- **机型视图**（左侧导航独立入口，2026-09-07 注册；此前仅可经清单页「机型视图」按钮进入）：按机型聚合的治具卡片墙（状态分布/呆滞数/封面图），共享样品侧机型主数据
- **新建申请备注**（2026-09-07）：整批「申请说明」（多行，对该批全部治具生效，落 `request_note`）+ 行级「备注」列（单条特有说明，落 `notes`）；详情弹窗分别展示「申请说明」与「备注」
...
- **项目看板**：kb-stat 统计卡（项目数/总任务/已完成/进行中/已延期/已取消）+ 类别/优先级分布 + 完成率 + 近 8 周完成趋势（CSS 柱状图）
...
- **状态机管理**（仅 ADMIN）：可视化编辑 5 态标签/颜色（含 CANCELLED 已取消终态）+ 转移规则，保存即时生效
...
### v3 状态机修复与 PLM 升级（2026-09-08）

- **状态机修复（OVERDUE 回归派生态）**：取消流转事务内 OVERDUE 物理写回（该缺陷导致延期任务被永久锁死）；已延期任务可「继续处理(RESUME) / 补录完成(FINISH) / 取消(CANCEL)」，迁移自动回迁存量物理 OVERDUE 行并失效旧版死锁拓扑配置
- **新转边**：IN_PROGRESS→NOT_STARTED（BACK 退回）、DONE→IN_PROGRESS（REOPEN 重新打开）补齐后端边，前端快捷条/看板拖拽/列表行内按钮同步
- **CANCELLED 终态**：NOT_STARTED/IN_PROGRESS/OVERDUE → CANCELLED（仅 PM/ADMIN），终态不可再流转；看板统计新增「已取消」卡（cancelled_count）
- **甘特真实跨度**：任务新增 start_date（计划开始日，可空）；条形起于 start_date，无开始日回退「截止前 7 天」估算；创建/编辑弹窗支持填写
- **风险/变更↔任务互链**：project_risks/project_changes 新增 task_id，识别/编辑弹窗选关联任务（同项目校验）；任务详情新增「风险/变更」tab 双向展示
- **附件受控下载**：新增 `GET /api/projects/tasks/:tid/files/:fid/download`（登录 + 任务相关人校验）；旧静态路径 /uploads/projects/* 兼容保留（渐进切换）
- **交互补齐**：统一空态组件 .pk-empty（看板列/列表/详情 tab/风险/变更/甘特）；移动端适配（≤767px 看板横向滑动、筛选堆叠、甘特/图谱横滚）
- **routes-tasks.js 拆分**：编辑/删除/批量域拆至 routes-task-edit.js（22.6k→14.8k 字符，回到红线内）；manifest 补声明 9 张 OA 表 + schema.sql 同步

### 设备导入追踪（2026-09-08）

- **标准模板内置**：「设备导入标准流程」模板（评估→方案制定→设计评审→采购制造→验收效益 5 阶段任务 + 3 里程碑），模板页一键实例化，偏移天数自动换算计划日期
- **图纸附件**：任务附件白名单新增 CAD 图纸格式（dwg/dxf/step/stp），单文件上限 10MB→50MB（旧格式与存量附件完全兼容；nginx client_max_body_size 需 ≥50m）
- **效益追踪**：项目扩展信息新增「预期效益 / 实际效益备注」，项目详情弹窗「预算/成本/效益」卡展示与编辑（权限同预算：ADMIN/PM/owner）；BUDGET 变更联动等旧调用不受影响（未传字段保留库内值）
- **当前阶段**：项目详情弹窗信息卡派生显示「当前阶段」（最早未达成里程碑，只读）

### 管制扫码入库（2026-09-09）

- **扫码入库台**：管制子系统新增「扫码入库」导航（CUSTODY/ME 可见，ADMIN 兜底）；扫管制标签二维码（内容=管制单号）或手动输入 → 回显单据关键信息 → **填写库位（必填）** → 确认入库（REWORK_REPORTED → REIN_STOCK，写 in_stock_at 与库位，留痕「扫码入库 · 库位:x」）
- **权限收紧**：扫码通道单独校验「仓库人员」（CUSTODY 且部门=仓库/资材部，跟随 control-flow.json deptAliases 别名）或 ADMIN；详情页原「入库」按钮通道不受影响（双通道并存）
- **连扫**：入库成功自动清空单号并回焦，支持扫码枪连续作业；本次会话保留最近 10 笔记录
