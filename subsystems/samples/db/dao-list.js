// subsystems/samples/db/dao-list.js — 样品查询域 DAO（2026-09-05 自 dao.js 按域拆分，接口经 dao.js 合并导出不变）
...
module.exports = function createDaoList(deps) {
  ...
  function _listWhere(opts) {
    ...
    // 2026-09-05 角色化列表：mine_uid=当前用户 id → 只看「我创建的」（服务端派生，路由层注入，客户端不可伪造他人 uid）
    if (opts.mine_uid) { where.push('created_by = ?'); params.push(opts.mine_uid); }
    // 2026-09-05 角色化列表：领出超时未归还（口径与 listCheckoutOverdue 一致）
    if (opts.checkout_overdue === '1') { where.push("status='CHECKED_OUT' AND expected_return_at IS NOT NULL AND " + ISO_RET + " < " + NOW_UTC); }
    return { where: where, params: params };
  }

  // 列表 ORDER BY（sort 白名单：默认最新/created_at/sample_no 为旧值；mine/inspect/status 为 2026-09-05 角色档新增）
