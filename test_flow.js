...
(async () => {
  ...
  // 1) 研发新建样品（model/source_type/station 必填：机型强制校验 2026-08-05 新增）
  const mk = await call('POST', '/api/samples', { name: '测试样品-X', spec: 'SPEC-X', model: 'SF1225', station: '马达组', source_type: 'T' }, 'rd');
  ...
  // 12) 研发新建另一个样品（model/source_type/station 必填：机型强制校验）
  const rdInfo = await call('GET', '/api/me', null, 'rd');
  const rdId = String(rdInfo.data.id);
  const mk2 = await call('POST', '/api/samples', { name: '测试样品-Y', spec: 'SPEC-Y', model: 'SF1225', station: '马达组', source_type: 'T' }, 'rd');
